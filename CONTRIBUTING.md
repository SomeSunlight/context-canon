...
ContextCanon currently uses four real Context Nodes on its own repository:
...
A review candidate may intentionally carry understood generated drift while the project owner is still shaping the result. A **merge candidate** is not complete until the committed self-hosted generated packages match current compiler output.

## Branch and merge cadence

`main` is the last project-owner-reviewed, fully reproducible ContextCanon stage. Work on one coherent product block per branch/PR and use three distinct boundaries:

- **review-ready:** the large line is coherent, relevant tests have exposed obvious breakage, and any known CI/drift is disclosed;
- **merge-ready:** after project-owner approval, generated packages are regenerated once from the final authored state, the exact head is fully green with zero drift, and the final diff is checked;
- **baseline-closed:** after the successful merge, durable status surfaces record the actual merge outcome and no longer describe the preceding review candidate as current.

Before squash-merging, require:

- documentation matches implemented behavior;
- `STATE.md` describes the intended accepted project situation clearly enough to be useful after merge;
- `PLAN.md` makes the completed work and remaining merge transition explicit;
- compiler-owned generated packages were regenerated by the compiler itself;
- deterministic positive and negative tests pass;
- `contextcanon check --all .` is clean.

Immediately after squash-merging and before starting another product block, close the accepted-baseline checkpoint:

- verify the actual merge result from GitHub/repository metadata;
- mark the preceding merge transition complete in `PLAN.md` when it was tracked there;
