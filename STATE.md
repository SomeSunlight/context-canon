...
PR #31 established the 0.8.0 first-adoption baseline; PR #50 then advanced the public package/CLI to 0.8.1 with actionable Windows `WinError 5` scanner-lock guidance while preserving the onboarding semantics. Draft PR #54 is now the current unaccepted ContextCanon 0.9.4 owner-test candidate. Issues #55–#59 hardened installation, PLAN-first inventory review, embedded Git-directory handling, Windows diagnostics, durable inventory restore, and live-project reset/documentation UX. Issue #60 hardens the first semantic handoff: the STEP-03 Evidence snapshot is now immutable run identity before any semantic workspace/PLAN refresh; structured textual Evidence such as CSV/JSON/YAML may remain first-class knowledge bodies/resources without Markdown conversion; and IDE/agent semantic reviewers are explicitly directed to a separate scratch project containing only the generated instruction + frozen Evidence. Future optional LLM inventory triage is planned to derive candidate descriptions progressively from titles/intros or structured headers/top-level keys before reading deeper. The 0.9.4 candidate passes the complete deterministic suite (293 tests), self-hosted build/check with zero generated drift, and diff hygiene.
