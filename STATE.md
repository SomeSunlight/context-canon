...
PR #31 established the 0.8.0 first-adoption baseline; PR #50 then advanced the public package/CLI to 0.8.1 with actionable Windows `WinError 5` scanner-lock guidance while preserving the onboarding semantics. Draft PR #54 is the current unaccepted ContextCanon 0.9.2 owner-test candidate. Its first Hello World Extended owner walkthrough produced Issues #55/#56, and the first real-project inventory run produced Issue #57 for embedded Git-directory handling and Windows Errno-13 diagnostics. The next owner finding, Issue #58, separates transient onboarding work from durable inventory decisions: `onboard init` maintains a bounded project `.gitignore` block, the visible workspace and heavy Evidence snapshots stay ignored, while compact `inventory-state.json` and `inventory-acceptance.json` remain Git-visible so a clone can reconstruct reviewed file classifications and later drift. The 0.9.2 candidate passes the complete deterministic suite (285 tests), self-hosted build/check with zero generated drift, and diff hygiene.
