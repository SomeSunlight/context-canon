...
PR #31 established the 0.8.0 first-adoption baseline; PR #50 then advanced the public package/CLI to 0.8.1 with actionable Windows `WinError 5` scanner-lock guidance while preserving the onboarding semantics. Draft PR #54 is now the current unaccepted ContextCanon 0.9.5 owner-test candidate. Issues #55–#60 hardened installation, inventory/recovery UX, embedded Git handling, immutable Evidence identity, structured-data parity, and isolated semantic-agent guidance. Issue #61 turns that guidance into a deterministic model-neutral Semantic Handoff Workspace harness: STEP 04 and STEP 08 now each receive a fresh disposable directory plus equivalent ZIP containing only exact frozen Evidence at repository-relative paths and a small `.contextcanon-handoff/` control surface. Result import is explicit, exact-text preserving and still followed by the existing deterministic validator/human gate; later raw LLM artifacts are not carried into subsequent semantic handoffs. Future interpretation/reasoning steps are expected to register the same harness. The 0.9.5 candidate passes the complete deterministic suite (297 tests), self-hosted build/check with zero generated drift, and diff hygiene.
