...
PR #31 established the 0.8.0 first-adoption baseline; PR #50 then advanced the public package/CLI to 0.8.1 with actionable Windows `WinError 5` scanner-lock guidance while preserving the onboarding semantics. Draft PR #54 is now the current unaccepted ContextCanon 0.9.3 owner-test candidate. Issues #55–#58 hardened installation, PLAN-first inventory review, embedded Git-directory handling, Windows diagnostics, Git persistence, and inventory restore. Issue #59 adds the missing live-project recovery layer: `contextcanon onboard reset . --from <STEP>` now covers STEP 01–12, with reset/restart shown before STEP 01 in the generated PLAN; user onboarding commands live in `docs/onboarding-cli.md`, while framework-only onboarding architecture/trust contracts live in `nodes/internal/framework-development/docs/onboarding-design.md`. The walkthrough now models project deliveries → triage → frozen Evidence → shelf design → placement and explicitly marks the future interpretation stage as coming soon. The 0.9.3 candidate passes the complete deterministic suite (291 tests), self-hosted build/check with zero generated drift, and diff hygiene.
