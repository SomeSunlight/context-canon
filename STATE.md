...
PR #31 established the 0.8.0 first-adoption baseline; PR #50 then advanced the public package/CLI to 0.8.1 with actionable Windows `WinError 5` scanner-lock guidance while preserving the onboarding semantics. Draft PR #54 is the current unaccepted ContextCanon 0.9.1 owner-test candidate. Its first Hello World Extended owner walkthrough produced Issues #55/#56: installation now leads into `onboard init` and the generated PLAN, STEP 02 has a local inventory guide, opaque documents use explicit Markdown transcriptions, and ordinary source code is omitted from the default inventory. A subsequent real-project inventory run exposed Issue #57: a Git directory/gitlink entry was opened as a file and surfaced as generic Windows Errno 13, which the CLI then misleadingly decorated with WinError-5/scanner guidance. The 0.9.1 correction handles such directory entries explicitly, distinguishes generic permission failures from WinError 5, and passes the complete deterministic suite (283 tests), self-hosted build/check with zero generated drift, and diff hygiene.
