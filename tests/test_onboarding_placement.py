...
class OnboardingPlacementTests(unittest.TestCase):
    ...
    def test_instruction_is_bound_to_human_structure_and_preserves_source_language(self):
        ...
        self.assertIn("`README.md` as human/customer first-contact orientation", instruction.text)
        ...
        self.assertIn("structure-preserving reduction, not generic prose summarization", instruction.text)
        self.assertIn("shorter/simpler table", instruction.text)
        self.assertIn("keep an ordered list", instruction.text)
        self.assertIn("diagram or ASCII structure", instruction.text)
        self.assertIn("representative example", instruction.text)
        self.assertIn("comma/semicolon snake sentence", instruction.text)
        self.assertIn("`CONTRIBUTING.md` should primarily serve humans who want to contribute", instruction.text)
        self.assertIn("Harness-specific files such as `.goosehints`, `AGENTS.md`, `CLAUDE.md`", instruction.text)
        self.assertIn("adapter/projection surfaces, not semantic ownership boundaries", instruction.text)
        self.assertIn("promote harness-independent meaning into canonical Context Nodes", instruction.text)
        self.assertIn("Make such adapters as thin as the harness safely allows", instruction.text)
        self.assertIn("A non-Markdown harness adapter is not rewritten by `source_edits`", instruction.text)
