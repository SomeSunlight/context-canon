...
for old, new in replacements.items():
    if plan.count(old) != 1:
        raise SystemExit(f"PLAN checkpoint target count={plan.count(old)} for {old[:70]!r}")
    plan = plan.replace(old, new, 1)

anchor = "- [x] Validate preview/publication vertically on the real materialized `ai-workstation` Nodes with at least one Overview, root Rule, child-local Rule, Topic/Resource, fixed-authority mapping, mutable-document follow-up and reusable Development Workflow Source.\n"
note = """
...
"""
if note.strip() not in plan:
    plan = plan.replace(anchor, anchor + note, 1)
...
"""
Path("STATE.md").write_text(state, encoding="utf-8", newline="\n")
