...
#### `GHL-001` — Use GitHub Local development objects

Use the configured `github.local` provider for workflow Issue, Pull Request, review, checks and merge operations; use the local Git repository for branches, commits, diffs and working-tree changes.

#### `GHL-002` — Keep Issues visible project documentation

Persist local Issues as ordinary visible project documents under `issues/`; do not hide their canonical content in ContextCanon machine state or an opaque runtime database.

#### `GHL-003` — Do not silently switch providers

If the configured `github.local` runtime is unavailable or lacks a required operation, fail clearly rather than silently creating equivalent objects in github.com, Jira, Bitbucket, or another external system.
...
- [`CONTEXT/references/63b92022-db1a-4875-be99-017726cbfce7/nodes/library/github-local/docs/provider.md`](CONTEXT/references/63b92022-db1a-4875-be99-017726cbfce7/nodes/library/github-local/docs/provider.md)
