...
def _render_contract() -> list[str]:
    return [
        ...
        "7. Separate **Nodes** from larger knowledge bodies that should remain documents/corpora/data resources rather than be rewritten into Node prose. Any UTF-8 textual file present in frozen Evidence may be a knowledge-body path, including Markdown, CSV, JSON, YAML and similar structured text. Use `project-documentation` for ordinary project-owned bodies that may later be reorganized, `authoritative-reference` for material whose wording/data authority must remain fixed, and `imported-corpus` for fixed imported sets. Do **not** conclude that structured data is ineligible merely because it is not Markdown. Opaque binary authorities such as PDF remain outside direct semantic handling unless the owner supplied a faithful textual transcription.",
        ...
        "- `paths` contains only files present in the frozen Evidence and may include Markdown or structured textual data such as CSV/JSON/YAML; it may be empty when the authoritative body is only referenced/described by frozen Evidence rather than included in it;",
