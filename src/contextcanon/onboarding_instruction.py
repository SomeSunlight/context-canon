...
def _render_contract() -> list[str]:
    return [
        ...
        "",
        "Analyze the frozen evidence as an onboarding reviewer. Your task is classification and proposal, not implementation.",
        "",
        ...
        "2. Read all listed evidence before classifying. Evidence selection is intentionally conservative; absence from the snapshot is not evidence that something does not exist in the live repository.",
        "3. Every proposal item must cite one or more exact evidence references with repository path, the listed SHA-256, and inclusive 1-based line ranges that support the item.",
        "4. Do not turn descriptive prose into a Rule merely because it sounds important. `local-rule` is for durable prescriptive project governance.",
        "5. Keep temporary status and planned work in `state-planning`; never promote temporary reality into reusable governance.",
        "6. Preserve useful README, CONTRIBUTING, architecture, operating, and explanatory documents as `ordinary-documentation` or `topic-resource` when that is their natural role. Onboarding is not destructive migration.",
        "7. Before using `candidate-reusable-node`, compare the practice against every supplied catalog package. Do not duplicate an existing reusable Source merely to rephrase it locally.",
        "8. Use `existing-source` only for a supplied catalog entry and copy its exact `source_node_id` and `source_name`. Do not claim that the Source is accepted; this is only a proposal for later human review.",
        "9. A practice may be reusable even when this repository is the only evidence available. Mark uncertainty in confidence and rationale instead of pretending broader adoption has been proven.",
        "10. Surface contradictions and important missing decisions as `unresolved-question`. Do not silently reconcile incompatible evidence.",
        "11. Prefer a small set of high-value, non-duplicative items over paraphrasing every sentence in the repository.",
        "12. Do not create, edit, move, or delete repository files. Do not emit canonical ContextCanon Markdown. This stage produces a proposal only.",
        "",
        "## Output contract",
        "",
