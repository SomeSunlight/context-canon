...
def _render_contract() -> list[str]:
    return [
        ...
        "5. Every proposal item must cite one or more exact evidence references with repository path, the listed SHA-256, and inclusive 1-based line ranges that support the item.",
        "6. Do not turn descriptive prose into a Rule merely because it sounds important. `local-rule` is for durable prescriptive project governance.",
        "7. Keep temporary status and planned work in `state-planning`; never promote temporary reality into reusable governance.",
