...
from .model import NodeMetadata, ParsedNode, Rule, SourceRef, Topic, TopicTarget
...
def parse_node(node_root: Path, repo_root: Path | None = None) -> ParsedNode:
    ...
    _reject_unsupported_changes(lines, sections.get("Changes"), source_path)
    ...
    return ParsedNode(node_root, repo_root, metadata, tuple(sources), tuple(rules), tuple(topics))


def _reject_unsupported_changes(lines: list[str], section: tuple[int, int] | None, source_path: Path) -> None:
    if not section:
        return
    start, end = section
    meaningful = [
    ...
    if meaningful:
        raise ContextCanonError(
            f"{source_path}: ## Changes is reserved but not implemented in Walking Skeleton 1"
        )
