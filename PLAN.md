...
- [ ] Make authored documentation ownership unambiguous: put reusable/generic guidance with the reusable Node that owns it, framework-internal material with Framework Development, keep generated `CONTEXT/references/` copies clearly identifiable as generated package material, and preserve useful wording/information while removing accidental duplicate maintenance surfaces.
- [ ] Correct the reusable Foundation boundary exposed by the first consolidation pass: Foundation-owned Topic resources must be authored with Foundation; Framework Development may reference those resources but must not become their hidden owner or keep duplicate authored copies.
