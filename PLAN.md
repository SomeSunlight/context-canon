...
##### S — Real machine-state upgrade hardening
...
##### T — Final publication readability and Node discoverability

Goal: finish PR #13 with a publishing surface that explains composition at first glance. This block changes presentation and discoverability only; it must not weaken package pinning, inheritance boundaries, Source semantics, or the rule that the project owner performs the final merge.

**Status: ACTIVE — final pre-publication polish.**

- [ ] 1. Make the authoring source self-explanatory: render the direct relationship as `Parent Context Node` immediately after the Node identity header and before local content; use `Local Overview`, `Local State`, `Local Plan`, `Local Rules`, and `Local Topics` for content authored in this Node. `Local` means authored here, not an implicit override. Preserve compatibility with existing authored headings during migration so current projects are not broken merely by the wording cleanup.
- [ ] 2. Make generated Official Context composition visible at the top: show the direct Parent Context Node and an effective imported-context list that makes every inherited semantic origin visible, including indirect ancestors and reusable Sources, with Node name, version, relation/provenance, and a link to an exact local accepted reference/package copy where one can inspect why that context applies. Do not list unrelated siblings and do not require network access.
- [ ] 3. Keep provenance auditable through the whole chain: the composition summary must make different accepted versions distinguishable and must remain deterministic from the exact accepted Parent/Source packages; no live Parent/Source dereference may be introduced merely to improve presentation.
- [ ] 4. Add a small ContextCanon Node `README.md` doorplate wherever a Node directory has no project-owned README and ContextCanon can prove it owns the generated placeholder. It should explain what the directory is, point to `CONTEXT.md` as Official Context and `CONTEXT.src.md` as the authoring source, link to the ContextCanon project/documentation, and contain no duplicate canonical project semantics. Never overwrite or silently adopt a foreign/project-authored README.
- [ ] 5. Carry the existing onboarding documentation follow-up into this final polish: document that book placement itself often exposes hidden responsibilities, boundaries and unresolved questions, and that concise placement finding titles already form a useful project index before deeper material is opened.
- [ ] 6. Add focused regressions for heading migration/order, effective-context composition/provenance, sibling exclusion, offline rendering, and README ownership/collision safety. Regenerate self-hosted Context output, run the complete deterministic suite plus `contextcanon build --all .`, `contextcanon check --all .`, and diff hygiene, then update STATE/PR review wording.
